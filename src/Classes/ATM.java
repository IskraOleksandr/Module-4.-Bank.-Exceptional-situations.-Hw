...
public class ATM {
    public List<Moneybox> moneyBoxes = new ArrayList<>();
    ...
    public void addMoneyBox(int nominal,int count){
        try {
            if (moneyBoxes.size() <= maxNumberMoneyBoxes) {
                Moneybox m = new Moneybox(nominal, count);
                moneyBoxes.add(m);
            } else {
    ...
    public void minusCount( Moneybox mb, int number) {
    ...
    public void plusCount( Moneybox mb,  int number) {
        int newCount = mb.getCount() + 1;
    ...
    public int takeMoneyFromBox(int summa, int nominal){
        ...
        if(number1 > 0){
            Moneybox moneybox = findMoneyBox(nominal);
            if (moneybox != null){
                boolean have = true;
                int numberCoupure = 0;
                while(number1 > 0 && have) {
                    try {
                        minusCount(moneybox,numberCoupure);
                        numberCoupure++;
                        number1--;
    ...
    public Moneybox findMoneyBox(int nominal){
        for (var m :moneyBoxes ) {
        ...
        if(nominal ==1 ||nominal ==2 ||nominal ==5 ||nominal ==10 ||nominal ==20 ||
            ...
            if(count > 0){
                ...
                if (moneybox != null){
                    ...
                    while(count > 0 && have) {
                        try {
                            plusCount(moneybox,numberCoupure);
                            numberCoupure++;
                            count--;
                    ...
                    if( success) {
                        ...
                        try{
                            ...
                            if(choise==1){
                                ...
                                int coun=moneybox.getCount();
                                moneybox.setCount(coun-numberCoupure);
